...
//-------------------------------------------------------------ROUTE: 1-------------------------------------------------------------
// created an api for user registration /auth/createuser
router.post(
  "/createuser",
  [
    body("name").isLength({ min: 2 }), // name length should be minimum of 2 characters
    body("username").isLength({ min: 2 }), // name length should be minimum of 2 characters
  ...
  async (req, res) => {
    ...
    try {
      // if there is no error in the data that user has returned then run this try block
      let user = await User.findOne({ username: req.body.username,
                                      email: req.body.email});
      if (user) {
        // check that the user with the username  does not already exist in the database
        return res
          .status(400)
...
//-------------------------------------------ROUTE: 2-----------------------------------------------------------------------------------------------------
// authenticate a user with /auth/login
router.post(
  "/login",
  [
  ...
  async (req, res) => {
    ...
    const { username, password } = req.body; // destructure email and password form req.body
    try {
      let user = await User.findOne({ username });
      if (!user) {
        return res
          .status(400)
          .json({ error: "please try ot login with correct credentials" });
      }
...
//-------------------------------------------ROUTE: 3-----------------------------------------------------------------------------------------------------
// get logged in user detail using post /auth/getuser
